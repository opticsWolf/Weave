...
class NodeProxyWidget(QGraphicsProxyWidget):
    """Proxy that notifies its parent ``NodeWidgetHost`` when Qt adds or
    removes a child item (combo-box popup rendering).

    ``NodeWidgetHost`` uses these callbacks to temporarily raise its own
    z-value so the popup paints above sibling items (ports, resize handle).
    """

    def __init__(self, host: 'NodeWidgetHost'):
        super().__init__(host)
        self._host = host
        self._popup_children: int = 0
        self.setFlag(QGraphicsItem.GraphicsItemFlag.ItemSendsGeometryChanges, True)

    def itemChange(self, change, value):
        if change == QGraphicsItem.GraphicsItemChange.ItemChildAddedChange:
            self._popup_children += 1
            if self._popup_children == 1:
                self._host._on_popup_open()

        elif change == QGraphicsItem.GraphicsItemChange.ItemChildRemovedChange:
            self._popup_children = max(0, self._popup_children - 1)
            if self._popup_children == 0:
                self._host._on_popup_close()

        return super().itemChange(change, value)


class NodeWidgetHost(QGraphicsItem):
    """Transparent host for the ``QGraphicsProxyWidget`` that contains the
    node's embedded Qt widgets (``WidgetCore``).

    Lives as a direct child of ``Node`` — a sibling of ``NodeHeader``,
    ``NodeBody``, ports, and the ``ResizeHandle``.  This means its z-value
    can be raised above those siblings when a combo popup opens, which was
    impossible when the proxy lived inside ``NodeBody``'s subtree.

    This item has **no paint output**.  It exists solely to own the proxy
    and participate in the scene's z-ordering.
    """

    __slots__ = (
        '_node', '_proxy', '_widget', '_saved_z', '_rect',
    )
    ...
    def __init__(self, parent: 'Node'):
        ...
        self._proxy = NodeProxyWidget(self)
    ...
    def update_layout(
        self, widget_y: float, widget_h: float, body_width: float,
    ) -> None:
        """Reposition and resize the proxy within the body area.

        Coordinates are in body-local space (this item is positioned at
        the same origin as ``NodeBody``).

        Args:
            widget_y:   Y offset inside the body for the widget area.
            widget_h:   Available height for the widget.
            body_width: Full width of the body.
        """
        if self.scene():
            self.prepareGeometryChange()
        self._rect = QRectF(10, widget_y, max(0, body_width - 20), max(0, widget_h))
        self._proxy.setPos(10, widget_y)
        self._proxy.resize(max(0, body_width - 20), max(0, widget_h))

    def get_content_min_size(self) -> Tuple[float, float]:
        """Minimum size required by the content widget (with padding)."""
        hint = self._widget.minimumSizeHint()
        return hint.width() + 20, hint.height()

    def set_content(self, widget: QWidget) -> None:
        """Replace the content widget inside the proxy root."""
    ...
    def _on_popup_open(self) -> None:
        if self._saved_z is None:
            self._saved_z = self.zValue()
            self.setZValue(10_000)

    def _on_popup_close(self) -> None:
        if self._saved_z is not None:
            self.setZValue(self._saved_z)
            self._saved_z = None
        # Force Qt to re-query boundingRect so the stale popup area
        # no longer intercepts hit-testing.
        self.prepareGeometryChange()

    # ── QGraphicsItem boilerplate ────────────────────────────────────

    def boundingRect(self) -> QRectF:
        return self._rect

    def shape(self) -> QPainterPath:
        """Constrain hit-testing to the actual widget area."""
        path = QPainterPath()
        path.addRect(self._rect)
        return path

    def paint(self, painter, option, widget=None):
        pass  # Transparent — no visual output


class NodeBody(QGraphicsItem):
    """
    Body component responsible for painting the node body background,
    outline, gradient, and port area indicators.

    Widget hosting has been extracted into ``NodeWidgetHost``.
    """
    
    __slots__ = (
        '_node', '_width', '_height', '_bg_color', '_outline_color',
        '_shape_path', '_outline_path',
    ...
    def update_layout(self, width: float, height: float, 
        ...
        """
        Updates the geometry of the body (paint area only).
    ...
    def set_size(self, width: float, height: float) -> None:
        ...
        Args:
            ...
            height (float): New height
        """
        self.update_layout(width, height, QRectF(), QRectF())
